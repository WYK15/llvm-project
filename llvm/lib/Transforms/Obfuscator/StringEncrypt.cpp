...
#define DEBUG_TYPE "stringEncrypt"

static cl::opt<bool> stringEncrypt("enable-strcry", cl::desc("substitute pass switch"), cl::init(false));

PreservedAnalyses StringEncryptionPass::run(Function &F, FunctionAnalysisManager &AM) { 
...
void StringEncryptionPass::HandleFunction(Function *Func) {
	//errs() << "StringEncryptionPass::HandleFunction on : " << Func->getName() << "\n"; 

	Constant *S = ConstantInt::get(Type::getInt32Ty(Func->getContext()), 0);
	GlobalVariable *GV = new GlobalVariable(
		*(Func->getParent()), S->getType(), false, GlobalValue::LinkageTypes::PrivateLinkage,
		S, "");
	encstatus[Func] = GV;	
...
GlobalVariable *StringEncryptionPass::ObjectivCString(GlobalVariable *GV, string name, GlobalVariable *oldrawString, GlobalVariable *newString, ConstantStruct *CS) {
  ...
  void StringEncryptionPass::HandleDecryptionBlock(BasicBlock *B, BasicBlock *C,
                             map<GlobalVariable *, pair<Constant *, GlobalVariable *>> &GV2Keys) {
    IRBuilder<> IRB(B);
